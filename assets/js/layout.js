...
(function () {
  ...
  var NAV = {
    front: [
      { key: 'landing',  label: 'الرئيسية',    icon: 'fa-solid fa-house',          href: 'alegria_landing_page.html' },
      { key: 'menu',     label: 'المنيو الرقمي', icon: 'fa-solid fa-utensils',     href: 'alegria_digital_menu.html' },
      { key: 'cart',     label: 'السلة',       icon: 'fa-solid fa-cart-shopping',  href: 'cart_checkout_alegria.html' },
      { key: 'checkout', label: 'إتمام الطلب', icon: 'fa-solid fa-credit-card',    href: 'checkout_alegria.html' },
      { key: 'tracking', label: 'تتبع الطلب',  icon: 'fa-solid fa-truck-fast',     href: 'customer_orders_tracking_alegria.html' },
      { key: 'contact',  label: 'تواصل معنا',  icon: 'fa-solid fa-envelope',       href: 'contact_us_alegria.html' }
    ],
    erp: [
      { key: 'exec',       label: 'لوحة التشغيل', icon: 'fa-solid fa-gauge-high',     href: 'alegria_erp_1.html' },
      { key: 'inventory',  label: 'المخزون',      icon: 'fa-solid fa-boxes-stacked',  href: 'alegria_erp_2.html' },
      { key: 'hr',         label: 'الورديات',     icon: 'fa-solid fa-clock-rotate-left', href: 'hr_shifts_alegria_erp.html' },
      { key: 'pos',        label: 'نقاط البيع',   icon: 'fa-solid fa-cash-register',  href: 'pos_alegria.html' },
      { key: 'kds',        label: 'المطبخ (KDS)', icon: 'fa-solid fa-fire-burner',    href: 'kds_kitchen_display_system.html' },
      { key: 'reports',    label: 'التقارير',     icon: 'fa-solid fa-chart-column',   href: 'reports_analytics_alegria_erp.html' }
    ],
    erpMore: [
      { key: 'purchasing',     label: 'المشتريات',          icon: 'fa-solid fa-truck-field',    href: 'purchasing_suppliers_alegria_erp.html' },
      { key: 'suppliers',      label: 'الموردون والحسابات', icon: 'fa-solid fa-handshake',      href: 'suppliers_alegria_erp.html' },
      { key: 'recipes',        label: 'الوصفات والتكاليف', icon: 'fa-solid fa-clipboard-list', href: 'recipes_costing_alegria_erp.html' },
      { key: 'roles',          label: 'مصفوفة الأدوار',    icon: 'fa-solid fa-shield-halved',  href: 'roles_matrix_alegria_erp.html' },
      { key: 'users',          label: 'المستخدمون',        icon: 'fa-solid fa-user-gear',      href: 'roles_permissions_alegria_erp.html' },
      { key: 'security',       label: 'سياسات الأمان',     icon: 'fa-solid fa-lock',           href: 'security_policies_alegria_erp.html' },
      { key: 'hr-employees',   label: 'الموظفون',          icon: 'fa-solid fa-users',          href: 'hr_employees_alegria_erp.html' },
      { key: 'hr-payroll',     label: 'الرواتب',           icon: 'fa-solid fa-money-check-dollar', href: 'hr_payroll_alegria_erp.html' },
      { key: 'treasury',       label: 'الخزينة',           icon: 'fa-solid fa-coins',          href: 'treasury_financials_alegria_erp.html' }
    ]
  };
  ...
  function brandHtml() {
    return '<a class="navbar-brand brand-logo me-3" href="' +
      (section === 'front' ? 'alegria_landing_page.html' :
        section === 'erp' ? 'alegria_erp_1.html' : '#') + '">' +
      '<img class="brand-img" src="' + 'images/logo.png" alt="' + brandName + ' - لوجو"/>' +
  ...
  function buildSidebar() {
    ...
    aside.innerHTML =
      ...
      '<a class="erp-link" href="../alegria_landing_page.html">' +
  ...
  function notifBell() {
    return '<div class="dropdown notif-wrap">' +
      ...
      '<a class="drop-item" href="alegria_erp_2.html">' +
      ...
      '<a class="drop-item" href="kds_kitchen_display_system.html">' +
      ...
      '<a class="drop-item" href="hr_shifts_alegria_erp.html">' +
  ...
  function profileMenu() {
    return '<div class="dropdown profile-wrap">' +
      ...
      '<a class="drop-item" href="hr_employees_alegria_erp.html">' +
      '<i class="fa-solid fa-address-card"></i>بيانات البروفايل</a>' +
      '<a class="drop-item" href="roles_permissions_alegria_erp.html">' +
      '<i class="fa-solid fa-gear"></i>إعدادات الصلاحيات</a>' +
      '<div class="drop-sep"></div>' +
      '<a class="drop-item rose" href="../login.html">' +
  ...
  function buildHeader() {
    ...
    if (section === 'front') {
      ...
      left = '<a class="btn btn-gold btn-sm rounded-pill px-3 header-menu-cta' + menuActive + '" href="alegria_digital_menu.html">' +
        '<i class="fa-solid fa-utensils"></i>المنيو الرقمي</a>';
    } else if (section === 'erp') {
      var erpHtml =
      ...
      left = '<a class="btn btn-ghost btn-sm" href="../alegria_landing_page.html"><i class="fa-solid fa-earth-americas me-1"></i>العودة للموقع</a>';
    }
    ...
    var html =
      ...
      '<i class="fa-solid fa-bars oc-burger" aria-hidden="true"></i>' +
      '<i class="fa-solid fa-xmark oc-burger-x" aria-hidden="true"></i></button>' +
  ...
  function buildOffcanvas(navHtml, left) {
    ...
    if (section === 'front') {
      ...
      inner = NAV.erp.map(offcanvasLink).join('') +
        ...
        '<a class="oc-nav-link" href="../../alegria_landing_page.html"><i class="fa-solid fa-earth-americas"></i>العودة للموقع</a>';
    } else {
      inner = '<a class="oc-nav-link" href="../alegria_landing_page.html"><i class="fa-solid fa-earth-americas"></i>العودة للموقع</a>' +
        '<a class="oc-nav-link" href="../login.html"><i class="fa-solid fa-right-to-bracket"></i>بوابة الإدارة</a>';
    }
  ...
  function syncBurger(off) {
    var burger = document.querySelector('.navbar-alegria .navbar-toggler');
    if (!burger || !off) return;
    off.addEventListener('show.bs.offcanvas', function () { burger.classList.add('showing'); });
    off.addEventListener('hide.bs.offcanvas', function () { burger.classList.remove('showing'); });
    off.addEventListener('hidden.bs.offcanvas', function () { burger.classList.remove('showing'); });
  }
  ...
  function buildTabbar() {
    ...
    if (current === 'privacy' || current === 'terms') return;
    ...
    if (section === 'front') {
      ...
      items = [
        { key: 'landing',  label: 'الرئيسية', icon: 'fa-solid fa-house',          href: 'alegria_landing_page.html' },
        { key: 'menu',     label: 'المنيو',    icon: 'fa-solid fa-utensils',      href: 'alegria_digital_menu.html' },
        { key: 'cart',     label: 'السلة',     icon: 'fa-solid fa-cart-shopping', href: 'cart_checkout_alegria.html', count: cartCount },
        { key: 'tracking', label: 'التتبع',    icon: 'fa-solid fa-truck-fast',    href: 'customer_orders_tracking_alegria.html' },
        { key: 'contact',  label: 'تواصل',     icon: 'fa-solid fa-envelope',      href: 'contact_us_alegria.html' }
      ];
    } else {
      items = [
        { key: 'exec',      label: 'التشغيل',     icon: 'fa-solid fa-gauge-high',    href: 'alegria_erp_1.html' },
        { key: 'inventory', label: 'المخزون',     icon: 'fa-solid fa-boxes-stacked', href: 'alegria_erp_2.html' },
        { key: 'pos',       label: 'نقاط البيع',  icon: 'fa-solid fa-cash-register', href: 'pos_alegria.html' },
        { key: 'reports',   label: 'التقارير',    icon: 'fa-solid fa-chart-column',  href: 'reports_analytics_alegria_erp.html' },
  ...
  function buildFooter() {
    ...
    var center =
      ...
      '<img class="footer-logo" src="' + 'images/logo.png" alt="' + brandName + ' - لوجو"/>' +
      ...
      '<a href="vip_support_24_7.html">الدعم الفني VIP</a>' +
      '<span class="sep">|</span>' +
      '<a href="privacy_policy_alegria.html">سياسة الخصوصية</a>' +
      '<span class="sep">|</span>' +
      '<a href="terms_of_use_alegria.html">شروط الاستخدام</a>' +
      '</div>' +
      '</div>';
